...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/09/05 04:44:12 by tsedigi           #+#    #+#             */
/*   Updated: 2018/09/05 04:44:13 by tsedigi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int					get_section_64(struct segment_command_64 *seg)
{
	...
	while (i < seg->nsects)
	{
		if (is_opt_set(get_ot_env(NULL)->opts, 't')
			...
			return (add_section_64(get_ot_env(NULL), sec));
...
static int					handle_loop_64(void *data,
	struct mach_header_64 *header, unsigned int i, t_ot_env *env)
{
	...
	while (i < header->ncmds)
	{
		if (lc->cmd == LC_SEGMENT_64)
			get_section_64((struct segment_command_64 *)lc);
		if (env->sections)
		{
			ot_display_all(env);
			return (EXIT_SUCCESS);
...
int							ot_handle_64(void *data)
{
	...
	if (is_opt_set(env->opts, 't') || is_opt_set(env->opts, 'd'))
		return (handle_loop_64(data, header, 0, env));
	return (EXIT_FAILURE);
}
