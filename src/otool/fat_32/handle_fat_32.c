...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/09/05 06:45:17 by tsedigi           #+#    #+#             */
/*   Updated: 2018/09/05 06:45:18 by tsedigi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_otool.h"

static void				mask_cpu(unsigned int to_mask)
{
	char				*conv;
...
static void				print_fat_arch(struct fat_arch *arch)
{
	...
	if (to_mask)
		...
		ft_putendl("0");
	ft_putstr("    offset ");
	ft_putnbr(swap_uint32(arch->offset));
	ft_putstr("\n    size ");
	ft_putnbr(swap_uint32(arch->size));
	ft_putstr("\n    align 2^");
...
static int				fat_print_header(struct fat_arch *arch,
	unsigned int narch)
{
	unsigned int		i;
	char				*conv;
	...
	conv = conver_base_unsigned(0,
		swap_uint32(get_ot_env(NULL)->magic), 16);
	ft_putendl(conv);
	conv ? ft_memdel((void **)&conv) : 0;
	ft_putstr("nfat_arch ");
	ft_putnbr(narch);
...
static int				fat_loop_arch(void *data,
	struct fat_arch *arch, unsigned int narch, unsigned int f)
{
	struct fat_header	*h;
	unsigned int		i;
...
int						handle_fat_32(void *data, unsigned int f)
{
	...
	if (is_opt_set(env->opts, 'f'))
		fat_print_header(arch, narch);
